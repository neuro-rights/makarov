function fig_hdl = E74
%   SYNTAX
%   E74
%   DESCRIPTION
%   This module accurately computes eddy currents and surface charges for a
%   conducting object in an external time-varying magnetic field of
%   arbitrary direction. The object may be a sphere, brick, cylinder, or an
%   external structure. The module also computes surface charge
%   distributions, eddy current density distribution in a plane, and eddy
%   current density at a point of interest.
%   
...
strbrick.sig       = 0.5;          %    conductivity of the brick
strbrick.par  = 1;                 %    uniform (0) or non-uniform (1) grid
strbrick.Tr  = 100;

%   Data for the cylinder (if 'cylinder' is selected) (uitable)
strcylinder.R          = 0.5;         %     cylinder radius, m
strcylinder.H          = 1.0;         %     cylinder height, m
...
strcylinder.sig        = 0.5;         %    conductivity of the cylinder
strcylinder.Tr         = 200;         %    approximate number of triangular patches
strcylinder.par        = 1;
...
strimport.az         = 0;         %     rotate about the z-axis, deg
strimport.sig        = 0.5;          %     conductivity of the imported mesh

%   Parameters for the incident H-field and external medium (uitable)
strei.Hinc = [0 0 -0.01/mu0];                           %   incidence direction and amplitude of the incident field, A/m
strei.sig = 0.0;                                        %   conductivity of the external medium
strei.freq = 1e4;                                       %   frequency in Hz
strei.normalize = sqrt(dot(strei.Hinc, strei.Hinc));    %   magnitude of the Hinc vector, A/m (updates automatically)

%   Visualization of the eddy current/E-field in the observation plane (uitable)
...
strop.planex = 0.00;         %   plane center: x in m
strop.planey = 0.0;         %   plane center: y in m
strop.planez = 0.30;        %   plane center: z in m
strop.planesizex   = 2.0;   %   plane length in m
strop.planesizey   = 2.0;   %   plane width in m
strop.divisionsx   = 17;    %   divisions with respect to length
strop.divisionsy   = 17;    %   divisions with respect to width
strop.arrow = 1.0;          %   relative arrow size versus default size
strop.Tr = 200;
strop.charges = 'yes';      %   include (yes) or not (no) charge plots
...
stroc.x           = 0.3;      %   x position in m
stroc.y           = 0.0;       %   y position in m
stroc.z           = 0.3;       %   z position in m
...
strout.J(1,1)          = 0;
strout.J(1,2)          = 0;
strout.J(1,3)          = 0;       %    Eddy current density at the observation point 
strout.B(1,1)          = 0;
strout.B(1,2)          = 0;
...
%%  Functions
    function outputgraphics
        ...
        strop.outyes         = zeros(size(Points, 1), 1);
        
        %%  Find the E-field/eddy current in a plane
        if strcmp(strop.yes, 'yes')      
            ...
            close(h);          
            msg = 1;
            B0 = mu0*repmat(strei.Hinc, length(strop.Points), 1);        
            dAdt = +0.5*2*pi*strei.freq*cross(B0, strop.Points, 2);        % changes
            temp = efield(strop.Points, c, P, t, Center, Area, normals, Size, R, eps0, msg);            
            E = -dAdt + temp;            
            Conductivity        = zeros(size(strop.Points, 1), 1);               
            Conductivity(find(~strop.outyes))= sig;            
            strop.J = repmat(Conductivity, [1 3]).*E;            
        end
        ...
        %%  Find the E-field/eddy current at observation point(s)
        strout.J          = 'none selected';
        strout.phi        = 'none selected';
        if strcmp(stroc.yes, 'yes')
            msg = 0;
            points = [stroc.x stroc.y stroc.z];
            B0 = mu0*strei.Hinc;        
            dAdt = +0.5*2*pi*strei.freq*cross(B0, points);                                  % changes    
            temp = efield(points, c, P, t, Center, Area, normals, Size, R, eps0, msg);
            E = -dAdt + temp;     
            Conductivity        = 0;
            outyes = inout(points, P, t, normals, Center, snumber);            
            if ~outyes Conductivity = sig; end;    
            strout.J = Conductivity*E;            
            phi = potential(points, c, P, t, Center, Area, normals, Size, R, eps0, msg);
            strout.phi = phi;            
        end         
        ...
        graphics_S72(io, strop, stroc, strei, P, t, c, objecttype);
    end
    ...
    function strout = clearoutput(strout)
        ...
        strout.J(1,1)          = 0;
        strout.J(1,2)          = 0;
        strout.J(1,3)          = 0;       %    Eddy current density at the observation point (if any)        
        strout.B(1,1)          = 0;
        strout.B(1,2)          = 0;
        strout.B(1,3)          = 0;       %    True B-field at the observation point
        strout.phi             = 0;       %    Scalar potential at the observation point (if any)
    ...
    function geometry(objecttype)        
        ...
        if strcmp(objecttype, 'import')
            h    = waitbar(0.5, 'Please wait - loading the mesh');
            ...
            if abs(strimport.ax)+abs(strimport.ay)+abs(strimport.az)>0
    ...
    function simulate()        
        ...
        Z = +Z*(sig - strei.sig)/(sig + strei.sig)/(4*pi*eps0);
        for m = 1:length(t)
            Z(m, m) = 1/(2*eps0);
        end
        close(h);
        
        %%   Incident field vector (center point integration given that E is constant)
        B0 = mu0*repmat(strei.Hinc, length(t), 1);        
        dAdt = +0.5*2*pi*strei.freq*cross(B0, Center, 2);                    % changes
        RHS  = -dAdt;
        V = (sig - strei.sig)/(sig + strei.sig)*sum(normals.*RHS, 2);        % changes        
        ...
        strout.J;               %    Eddy current density at the observation point (if any)
        ...
        strout.TotalCharge;     %    Total surface free charge, Wb
        strout
    ...
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E74-Eddy current solver for an object in an external magnetic field-MoM/BEM numerical solution', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function fConductingSphereCallback(~, ~)
            ...
            data = {...
                ...
                'Conductivity of the sphere, S/m',strsphere.sig;...
        ...
        function fConductingBrickCallback(~, ~)
            ...
            data = {...
                ...
                'Conductivity of the brick, S/m',strbrick.sig;...
        ...
        function fConductingCylinderCallback(~, ~)
            ...
            data = {...
                ...
                'Conductivity of the cylinder, S/m',strcylinder.sig;...
        ...
        function fConductingImportCallback(~, ~)
            ...
            data = {...                
                ...
                'Conductivity of the object, S/m',strimport.sig};
            ColumnName =   {'Name',   'Value'};
            columnformat    = {'char'};
            columneditable  =  [true];
            uitable('Units', 'normalized', 'Position', Position_table,...
        ...
        function ExternalCallback(~, ~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...
                'Conductivity of the external medium, S/m',strei.sig;...
                ...
                'Oscillation frequency, Hz', strei.freq};
            ColumnName =   {'Name',   'Value'};
            columnformat    = {'char'};
            columneditable  =  [true];
            uitable('Units', 'normalized', 'Position', Position_table,...
            ...
            function edit_callback(hObject, ~)
                data               = get(hObject,'Data');
                strei.sig          = data{6};
                strei.Hinc(1,1)    = data{7};
                strei.Hinc(1,2)    = data{8};
                strei.Hinc(1,3)    = data{9};
                strei.freq         = data{10};
        ...
        function fOutputSetupCallback(~, ~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...
                'Plot eddy currents in a plane',strop.yes;...
                ...
                'Include (yes) or not (no) surface charge plot', strop.charges}; 
            ColumnName =   {'Name',   'Value'};
            columnformat    = {'char'};
            columneditable  =  [true];
            uitable('Units', 'normalized', 'Position', Position_table,...
            ...
            function edit_callback(hObject, ~)
                data = get(hObject,'Data');
                strop.yes               = data{18};
                strop.potential         = data{19};
                strop.planetype         = data{20};
                strop.planex            = data{21};
                strop.planey            = data{22};
                strop.planez            = data{23};
                strop.planesizex        = data{24};
                strop.planesizey        = data{25};
                strop.arrow             = data{26};
                stroc.yes               = data{27};
                stroc.x                 = data{28};
                stroc.y                 = data{29};
                stroc.z                 = data{30};
                stroc.size              = data{31};
                strop.divisionsx        = data{32};
                strop.divisionsy        = data{33};
                strop.charges           = data{34};                
            end
            function B1_Callback(~, ~, ~)
                set(0, 'CurrentFigure', handles.figure1);
                if handles.simulate
        ...
        function fOutputDataCallback(~, ~)
            ...
            columneditable  =  [true];
            data = {...
                'Total surface electric charge, C', strout.TotalCharge;...
                'Eddy current Jx component at obs. point, A/m^2',strout.J(1);...
                'Eddy current Jy component at obs. point, A/m^2',strout.J(2);...
                'Eddy current Jz component at obs. point, A/m^2',strout.J(3);...                
                'Bx=mu*Hx component at obs. point, T',strout.B(1);...
                'By=mu*Hy component at obs. point, T',strout.B(2);...
                'Bz=mu*Hz component at obs. point, T',strout.B(3);...                
                'Scalar electric potential at obs. point, A',strout.phi;...
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...                    
            'String', 'This module accurately computes eddy currents and surface charges for a conducting object in an external time-varying magnetic field of arbitrary direction. The object may be a sphere, brick, cylinder, or an external structure. The module also computes surface charge distributions, eddy current density distribution in a plane, and eddy current density at a point of interest.');
...
function [ ] = graphics_S72(io, strop, stroc, strei, P, t, c, objecttype);
    ...
    %%   Draw magnetic field (incident only) in the observation plane
    if strcmp(strop.yes, 'yes')&(io == 1)
        strop.J = strop.J + 1e-12*max(max(strop.J))*rand(size(strop.J)); %   randomize a bit
        normalize = max(sqrt(dot(strop.J,strop.J,2)))/strop.arrow;
        fv = cone(strop.Points(:, 1), strop.Points(:, 2), strop.Points(:, 3), strop.J(:, 1), strop.J(:, 2), strop.J(:, 3), 36, 0.25, 1, normalize);
        patch(fv, 'FaceColor', 'y', 'EdgeColor', 'none');         
    end    
    if strcmp(strop.yes, 'yes')&(io == 0)
    ...
    if io == 0
        colormap jet;
        if ind<IND ec=[0.7 0.8 0.9]; else ec='none'; end
        if strcmp(objecttype, 'import') fc = [1 0.75 0.65]; ec='none'; else fc = [0 0.7 0]; end
        h = fill3(X, Y, Z, [0 0 1], 'FaceAlpha', 1.0, 'FaceColor', fc, 'EdgeColor', ec); 
        xlabel('x, m'), ylabel('y, m'), zlabel('z, m'); title('Problem geometry: conducting object under study and the incident H-field');
        ...
        if strcmp(strop.charges, 'yes')
            if strcmp(strop.yes, 'yes')
                C = cv(ttemp(1:3, :));    
                if ind<IND ec=[0.7 0.8 0.9]; else ec='none'; end
                h = fill3(X, Y, Z, C, 'FaceAlpha', 0.5, 'EdgeColor', ec);
                xlabel('x, m'), ylabel('y, m'), zlabel('z, m'); title('Solution: free surface charge density in C/m^2 and the resulting E-field')
                colorbar;
            else
                C = cv(ttemp(1:3, :));    
                h = fill3(X, Y, Z, C, 'FaceAlpha', 1.0, 'EdgeColor', 'w');
                xlabel('x, m'), ylabel('y, m'), zlabel('z, m'); title('Solution: free charge density in C/m^2')
                colorbar;            
            end
        else
            ec=[0.7 0.8 0.9]; 
            h = fill3(X, Y, Z, [0 0 1], 'FaceAlpha', 0.0, 'FaceColor', 'none', 'EdgeColor', ec); 
            xlabel('x, m'), ylabel('y, m'), zlabel('z, m'); title('Solution (surface charge density is not shown)');
        end
...
function [] = contourca(strop, Potential)
    ...
    if scale>1e-6
        ...
        xlabel('x, m'); ylabel('y, m'); zlabel('z, m'); grid on; axis equal; view(149, 25);
    else
