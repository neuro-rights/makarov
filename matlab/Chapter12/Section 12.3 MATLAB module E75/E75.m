function fig_hdl = E75
%   SYNTAX
%   E75
%   DESCRIPTION
%   This module accurately computes eddy currents and surface charges in a
%   conducting object generated by a helical coil. The object may be a
%   sphere, brick, cylinder, or an external structure. The module also
%   computes surface charge distributions, eddy current density
%   distribution in a plane, and eddy current density at a point of
%   interest.
%   
...
strcoil.center = [0 0 1.0];          %   coil center, m
strcoil.length = 0.10;               %   coil length, m
strcoil.radius = 0.10;               %   coil radius, m
strcoil.wireradius = 0.001;          %   coil wire radius, m
strcoil.turns   = 3;                 %   number of turns 
...
strop.divisionsx   = 23;    %   divisions with respect to length
strop.divisionsy   = 23;    %   divisions with respect to width
...
stroc.size        = 0.5;         %   relative marker size versus default size
...
%%  Functions
    function outputgraphics
        ...
        if strcmp(strop.yes, 'yes')      
            ...
            close(h);          
            msg = 1;             
            [dummy, A] = coil(strcoil, strop.Points);     
            dAdt = 2*pi*strei.freq*A;        % changes
        ...
        if strcmp(stroc.yes, 'yes')
            ...
            [dummy, A] = coil(strcoil, points);     
            dAdt = 2*pi*strei.freq*A;        % changes                                % changes    
        ...
        graphics_S72(io, strop, stroc, strei, P, t, c, objecttype, strcoil.ObsCONTOUR);
    end
    ...
    function cleaning
        ...
        %% Incident field
        strop.Points = Points;
        [strop.H, dummy] = coil(strcoil, strop.Points);                                   
    ...
    function geometry(objecttype)        
        ...
        %% Coil mesh(es)
        %   This function computes the wire mesh
    ...
    function simulate()        
        ...
        %%   Incident field vector (accurate integration of variable field)                   
        h = waitbar(0.5, 'Please wait - computing external field at interface(s)');
        V = zeros(size(t, 1), 1);
        [dummy, A]  = coil(strcoil, ObsPoint); 
        for n = 1:length(t)
            LocalA  = A(1+(n-1)*IndexS:n*IndexS, :);            
            dAdt    = 2*pi*strei.freq*LocalA;        % changes 
            RHS         = -dAdt; 
            temp        = sum(repmat(normals(n, :), IndexS, 1).*RHS, 2)';                     
            temp1       = sum(temp.*weightsS);
            V(n) = (sig - strei.sig)/(sig + strei.sig)*temp1;             
        end 
        close(h);
    ...
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E75-Eddy current solver for an object excited by a coil-MoM/BEM numerical solution', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function ExternalCallback(~, ~)
            ...
            data = {...
                ...
                'Coil wire radius (unused here), m',strcoil.wireradius;...
                ...
                'Frequency, Hz', strei.freq;...
                'Desired number of patches per circle (unused here)',strcoil.N;...
                'Mesh nonuniformity parameter (unused here)',strcoil.par};
            ColumnName =   {'Name',   'Value'};
            columnformat    = {'char'};
            columneditable  =  [true];
            uitable('Units', 'normalized', 'Position', Position_table,...
...
function [ ] = graphics_S72(io, strop, stroc, strei, P, t, c, objecttype, ObsCONTOUR);
    ...
    if io == 0
        ...
        h = fill3(X, Y, Z, [0 0 1], 'FaceAlpha', 0.3, 'FaceColor', fc, 'EdgeColor', ec); 
...
function [H, A] = coil(strcoil, r0);
    %   This function computes:
    %   Magnetic field of a coil (coil current I) for a point array r0
    %   Magnetic vector porential of a coil for a point array r0
    %   Coil is rotated first and is moved next
    %   SNM Spring 2014
    I        = strcoil.I;   
    center   = strcoil.center;
    ...
    for m = 1:turns
        ...
        [Aturn, Hturn] = staticsh(radius, radius, localcenter, r0, NumberOfCircleSubdivisions, nx, ny, nz);
        H = H + Hturn;
        A = A + Aturn;
    end
    H = I*H;
...
%   STATICSH finds magnetic field generated by an ellipse of a uniform current 
