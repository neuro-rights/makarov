function fig_hdl = E35
%   SYNTAX
%   E35
%   DESCRIPTION
%   This module models a 2D touchpad with a lattice of rows (top to bottom)
%   and columns (left to right) embedded into a dielectric. Default scheme:
%   A driving row electrode is assigned voltage V1=1V. All other conductors
%   including the human finger are assigned voltage V2=0V. Induced charge Q
%   on a sensing column electrode is expressed in the form of a "mutual
%   capacitance", C=Q/(V2-V1).
%   
...
input.RectSide      = RectSide;
input.RectSpacing   = RectSpacing;
input.TraceWidth    = TraceWidth;
input.SpacingZ      = SpacingZ;
...
strsc.positive = 0.05;         %   positive charge densities higher than this number times the average
%   positive charge density are assigned the same value
strsc.negative = 0.05;         %   negative charge densities smaller than this number times the average
...
strop.planez = 0.003;       %   plane center: z in m
...
%%  Functions
   function outputgraphics
    ...
    function geometry()        
        ...
        PD{1}(:, 3) = PD{1}(:, 3)-mean(PD{1}(:, 3));    %   center about zero
...
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        ChargeRow           =  strbrick.epsr*sum(Charge(t(:, 4)==input.Row));
        for m = 1:input.NumberOfPads            
            CapacitanceColumn(m) =  strbrick.epsr*sum(Charge(t(:, 4)==m+input.NumberOfPads))/(V2-V1);
        end       
        ...
        strout.CapacitanceColumn= CapacitanceColumn; 
        strout.ChargeRow        = ChargeRow;        
...
%% ---------------------------------------------------------------------------
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E35-Mutual-capacitance touchpad', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function fFirstParametersMenuCallback(~, ~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...
                'Rect. pad size, m',                        input.RectSide;...
                'Pad spacing, m',                           input.RectSpacing;...
                'Trace width, m',                           input.TraceWidth;...
                'XY grid separation, m',                    input.SpacingZ;...           
                'Number of triangles per row rectangle',    input.Triangles;...
                'Driven row number',                        input.Row};
            ...
            function edit_callback(hObject, ~)
                ...
                input.RectSide      = data{7};
                input.RectSpacing   = data{8};
                input.TraceWidth    = data{9};
                input.SpacingZ      = data{10};
                input.Triangles     = data{11}; 
                input.Row           = data{12};                 
            end
        ...
        function fSecondParametersMenuCallback(~, ~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            local = figure('Units', Position_units, 'Position', Position_local);
            set(local, 'Name', 'Finger phantom setting', 'NumberTitle', 'off', 'MenuBar', 'none');
            b1 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Update','Position', Position_b1, 'Parent', local, 'Callback', @B1_Callback);
            b2 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Cancel', 'Position', Position_b2, 'Parent', local, 'Callback', @B2_Callback);
        ...
        function fOutputDataCallback(~, ~)
            ...
            for m =1:input.NumberOfPads
                data{m, 1} = strcat('Capacitance to column #', num2str(m), ' pF');
                data{m, 2} = 1e12*strout.CapacitanceColumn(m);
            end        
            
            data1 = {...                     
                'Total charge of the driven row, C', strout.ChargeRow;...
                'Total charge of the system, C', strout.ChargeTotal;...
                ...
                'Number of triangular patches in the finger mesh',strout.PatchesF;...
...
%             web(HelpPath);
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...
            'String', 'This module models a 2D touchpad with a lattice of rows (top to bottom) and columns (left to right) embedded into a dielectric. Default scheme: A driving row electrode is assigned voltage V1=1V. All other conductors including the human finger are assigned voltage V2=0V. Induced charge Q on a sensing column electrode is expressed in the form of a "mutual capacitance", C=Q/(V2-V1).');
...
function [ ] = graphics_S32(io, strop, stroc, strei, P, t, c, Area, strsc, strbrick);
    ...
    if io == 0
        ...
        patch('Vertices', P, 'Faces', tM(:, 1:3), 'FaceColor',  'flat', 'FaceVertexCData', tM(:, 4), 'EdgeColor', 'none'); 
...
function [P, t] = lattice(input)
    %   SNM Fall 2012
    TopRectangles = input.NumberOfPads;
    RectSide      = input.RectSide;
    RectSpacing   = input.RectSpacing;
    TraceWidth    = input.TraceWidth;
    SpacingZ      = input.SpacingZ;
    Triangles     = input.Triangles;    %   Triangles per one row rectangle

    strbrick.L      = (TopRectangles-1)*RectSpacing + sqrt(2)*RectSide;         %   brick length, m
    strbrick.W      = sqrt(2)*RectSide;                                         %   brick width, m
    strbrick.Tr     = Triangles;                                                %   approximate # of triangles for top face

   
    strr.L           = RectSide*ones(TopRectangles, 1);               %   Rect. length, m         - first column
    strr.L(end+1)    = strbrick.L;
    strr.W           = RectSide*ones(TopRectangles, 1);               %   Rect. width, m          - second column
    strr.W(end+1)    = TraceWidth;
    if floor(TopRectangles/2)==TopRectangles/2
        temp                = [-(floor(TopRectangles/2)-1):1:floor(TopRectangles/2)]-0.5;
    else
        temp                = [-floor(TopRectangles/2):1:floor(TopRectangles/2)];
    end
    strr.x           = RectSpacing*temp;                              %   Rect. x-center, m       - third column
    strr.x(end+1)    = 0;
    strr.y           = zeros(TopRectangles+1, 1);                     %   Rect. y-center, m       - fourth column
    strr.l           = ones(TopRectangles+1, 1);                      %   Rel. edge length        - fifth column
    strr.no          = [1:TopRectangles+1];                           %   Domain # (0=diel.)      - sixth column
    ...
    %   Create mesh 
    [P, ttop, strr] = combmesh(str0, strr);
    %   Assign domain numbers
    t = combdomain(P, ttop, strr);
    t0 = t(t(:, 4)>0, :);

    %   Create the lattice
    Ptotal = [];
    ttotal = [];
    for m =1:TopRectangles
        PP = P;
        PP(:, 2) = PP(:, 2)+RectSpacing*(floor(TopRectangles/2)-0.5-m+1);
        ttemp = t0+length(Ptotal);
        ttemp(:, 4) = m;
        ttotal = [ttotal; ttemp];
        Ptotal = [Ptotal; PP];
    end
    ...
    %   Create the lattice at z = -SpacingZ;
    Ptotal1    = rotatez0(Ptotal, -90);
    if floor(TopRectangles/2)==TopRectangles/2
        Ptotal1(:, 1) = Ptotal1(:, 1) + RectSpacing/2;
        Ptotal1(:, 2) = Ptotal1(:, 2) + RectSpacing/2;
    end    
        
    Ptotal1(:, 3) = -SpacingZ;
    ttotal1    = ttotal;
    ttotal1(:, 4) = 2*TopRectangles - ttotal1(:, 4) + 1;

    ttemp = ttotal1+length(Ptotal);
    ttemp(:, 4) = ttotal1(:, 4);
    ttotal = [ttotal; ttemp];
    Ptotal = [Ptotal; Ptotal1];

    if floor(TopRectangles/2)==TopRectangles/2
        Ptotal(:, 1) = Ptotal(:, 1) - RectSpacing/2; 
    else
        Ptotal(:, 2) = Ptotal(:, 2) - RectSpacing/2; 
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [t] = combdomain(P, t, strr)
    %   Assign domain number for every rectangular subdomain
        ...
        for m = 1:M
            if m<M
                xv = [strr.x(m)-strr.L(m)/sqrt(2) strr.x(m) strr.x(m)+strr.L(m)/sqrt(2) strr.x(m) strr.x(m)-strr.L(m)/sqrt(2)];
                yv = [strr.y(m) strr.y(m)+strr.L(m)/sqrt(2) strr.y(m) strr.y(m)-strr.L(m)/sqrt(2) strr.y(m)];
            else
                xv = [strr.x(m)-strr.L(m)/2 strr.x(m)-strr.L(m)/2 strr.x(m)+strr.L(m)/2 strr.x(m)+strr.L(m)/2 strr.x(m)-strr.L(m)/2];
                yv = [strr.y(m)-strr.W(m)/2 strr.y(m)+strr.W(m)/2 strr.y(m)+strr.W(m)/2 strr.y(m)-strr.W(m)/2 strr.y(m)-strr.W(m)/2];
            end
            in = inpolygon(TriCenter(:, 1), TriCenter(:, 2), xv, yv);           
            t(in, 4) = strr.no(m); 
        end
    end

    function [P, t, strr] = combmesh(str0, strr)
        ...
        if strcmp(str0.type, 'c') [P, P0, edges0] = circle0(str0); end
        if strcmp(str0.type, 'r') [P, P0, edges0] = rectangle0(str0); end
        ...
        [Pr, edgesr, strr, P0, edges0] = combrectangle(str0, strr, P0, edges0);

        %%  Combine meshes (the boundary nodes are at the end here)   
        P      = [P; P0; Pr];                               %   all vertices
        edges  = [edges0+free; edgesr+free+size(P0, 1)];     %   all boundary edges
