function fig_hdl = E31
%   SYNTAX
%   E31
%   DESCRIPTION
%   This module accurately computes fields and polarization charges for a
%   dielectric object in an external electric field of arbitrary
%   direction. The object may be a sphere, a brick, or a cylinder. The
%   module also computes bound charge distributions, electric field
...
global objecttype; objecttype = 'cylinder';  %   either 'sphere' or 'brick' or 'cylinder' is allowed
...
strsphere.epsr      = 3.0;         %   relative dielectric constant of the sphere
...
strbrick.epsr       = 80;          %    relative dielectric constant of the brick
...
strcylinder.epsr       = 5.0;         %    relative dielectric constant of the brick
...
%%  Functions
    ...
    function simulate()        
        ...
        Z = +Z*(epsr - strei.epsr)/(epsr + strei.epsr)/(4*pi*eps0*strei.epsr);
        for m = 1:length(t)
            Z(m, m) = 1/(2*eps0*strei.epsr);
        end
        close(h);

        %%   Incident field vector (center point integration given that E is constant)
        V = (epsr - strei.epsr)/(epsr + strei.epsr)*sum(normals.*repmat(strei.Einc, length(t), 1), 2);
...
%% ---------------------------------------------------------------------------
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E31-Dielectric object in an external electric field-MoM/BEM numerical solution', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function fConductingCylinderCallback(~, ~)
            ...
            data = {...
                ...
                'relative dielectric constant of the cylinder',strcylinder.epsr;...
        ...
        function ExternalCallback(~, ~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            local = figure('Units', Position_units, 'Position', Position_local);
            set(local, 'Name', 'External electric field', 'NumberTitle', 'off', 'MenuBar', 'none');
            b1 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Update','Position', Position_b1, 'Parent', local, 'Callback', @B1_Callback);
            b2 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Cancel', 'Position', Position_b2, 'Parent', local, 'Callback', @B2_Callback);
            b3 = uicontrol('Style', 'Text', 'Units', 'normalized', 'String', {'External electric field';'Press ENTER after changing each parameter'}, 'Position', Position_b3, 'Parent', local, 'BackgroundColor', [0.8, 0.8, 0.8]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...
            'String', 'This module accurately computes fields and polarization charges for a dielectric object in an external electric field  of arbitrary direction. The object may be a sphere, a brick, or a cylinder. The module also computes bound charge distributions, electric field distribution in a plane, and electric field at a point of interest.');
