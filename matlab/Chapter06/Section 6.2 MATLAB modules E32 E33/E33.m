function fig_hdl = E33
%   SYNTAX
%   E33
%   DESCRIPTION
%   This module is an accurate MoM solution for combined metal-dielectric
%   structures with coincident faces. A parallel-plate capacitor is
%   simulated. The module also computes surface charge distributions,
%   electric field distribution in a plane, and electric field at a point.
%   
...
objecttype = 'brick';  %   'none' or 'brick' or 'cylinder' is allowed

%   Data for the brick (if 'brick' is selected) (uitable)
strbrick.L          = 0.01;         %    brick length, m
strbrick.W          = 0.01;         %    brick width, m
strbrick.H          = 0.002;        %    brick height, m
...
strbrick.epsr       = 10;           %    relative dielectric constant of the brick
strbrick.par  = 0.3;                %    uniform (0) or non-uniform (1) grid
strbrick.Tr  = 300;                 %   approximate number of triangular patches

%   Data for the cylinder (if 'cylinder' is selected) (uitable)
strcylinder.R          = 0.01;       %     cylinder radius, m
strcylinder.H          = 0.002;      %     cylinder height, m
...
strop.planesizex   = 0.015;   %   plane length in m
strop.planesizey   = 0.015;   %   plane width in m
...
strout.Canl         = 0;        %    Analytical capacitance in pF
strout.E(1,1)       = 0;
strout.E(1,2)       = 0; 
   ...
   function outputgraphics
    ...
    function geometry()        
        ...
        if strcmp(objecttype, 'brick')
            [PD, tD] = brick(strbrick.L, strbrick.W, strbrick.H, strbrick.Tr, strbrick.par);
            [PD] = rotatex(PD, strbrick.ax);
            [PD] = rotatey(PD, strbrick.ay);
            [PD] = rotatez(PD, strbrick.az);
            [centersD, normalsD] = normcenters(PD, tD);
            PD(:, 1) = PD(:, 1) + strbrick.X;
            PD(:, 2) = PD(:, 2) + strbrick.Y;
            PD(:, 3) = PD(:, 3) + strbrick.Z;
            centersD(:, 1) = centersD(:, 1) + strbrick.X;
            centersD(:, 2) = centersD(:, 2) + strbrick.Y;
            centersD(:, 3) = centersD(:, 3) + strbrick.Z;
            epsr = strbrick.epsr;
            contrast = (epsr - strei.epsr)/(epsr + strei.epsr);
            tD(:, 4) = 3; cD = zeros(size(tD, 1), 1); %   all dielectrics will have a fourth index 3
            %   select the metal caps
            tD(abs(normalsD(:,3)+1)<1e-12, 4) = 1;
            tD(abs(normalsD(:,3)-1)<1e-12, 4) = 2;                                   
        end
        if strcmp(objecttype, 'cylinder')
            [PD, tD]  = cylinder(strcylinder.R, strcylinder.H, strcylinder.Tr, strcylinder.par);
            [PD] = rotatex(PD, strcylinder.ax);
            [PD] = rotatey(PD, strcylinder.ay);
            [PD] = rotatez(PD, strcylinder.az);
            [centersD, normalsD] = normcenters(PD, tD);
            %   movement
            PD(:, 1) = PD(:, 1) + strcylinder.X;
            PD(:, 2) = PD(:, 2) + strcylinder.Y;
            PD(:, 3) = PD(:, 3) + strcylinder.Z;
            centersD(:, 1) = centersD(:, 1) + strcylinder.X;
            centersD(:, 2) = centersD(:, 2) + strcylinder.Y;
            centersD(:, 3) = centersD(:, 3) + strcylinder.Z;
            epsr = strcylinder.epsr;
            contrast = (epsr - strei.epsr)/(epsr + strei.epsr);
            tD(:, 4) = 3;  %   all dielectric will have a fourth index 3
            %   select the metal caps
            tD(abs(normalsD(:,3)+1)<1e-12, 4) = 1;
            tD(abs(normalsD(:,3)-1)<1e-12, 4) = 2;                       
        end
        ...
        %   Metal first
        NM      = sum(logic1) + sum(logic2);
        ND      = sum(logic3);
        t       = [tD(logic1, :); tD(logic2, :); tD(logic3, :)];
        P       = PD; 
        normals = [normalsD(logic1, :); normalsD(logic2, :); normalsD(logic3, :)];
        contrasts = contrast*ones(NM+ND, 1);                  
        strout.PatchesD = ND;
        strout.PatchesM = NM;
        c   = zeros(size(t, 1), 1);
        cD  = zeros(ND, 1);    
    ...
    function simulate()
        ...
        ZD = +ZD*(epsr - strei.epsr)/(epsr + strei.epsr)/(4*pi*eps0*strei.epsr);
        for m = 1:length(t)
            ZD(m, m) = 1/(2*eps0*strei.epsr);
        end
        close(h);
        
        %%  Construct the full MoM matrix
        Z(1:NM, :)          = ZM;
        Z(NM+1:NM+ND, :)    = ZD(NM+1:NM+ND, :);
        clear ZM;
        ...
        ChargeLower         = sum(Charge(t(:, 4)==1));    %   total charge of the first plate, C
        ChargeUpper         = sum(Charge(t(:, 4)==2));    %   total charge of the second plate, C
        ChargeDielectric    = sum(Charge(t(:, 4)==3));    %   total charge of the dielectric not in contact with metal, C
        ChargeTotal         = ChargeLower + ChargeUpper + ChargeDielectric;
        
        %   Finding free charges (Eq. (6.37) of Chapter 6)     
        cfree = zeros(NM, 1);
        for m = 1:NM
            cfree(m) = eps0*(epsr + strei.epsr)*sum(ZD(m, :).*c');
        end       
        Cnum                = sum(cfree(t(:, 4)==2).*Area(t(:, 4)==2))*1e12/(strge.phiu-strge.phi);       
        ...
        strout.Canl     = sum(Area(t(:, 4)==2))/(max(P(:, 3))-min(P(:, 3)))*epsr*eps0*1e12;
...
%% ---------------------------------------------------------------------------
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E33-Metal capacitor with a dielectric substrate (coincident faces)', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function fOutputDataCallback(~, ~)
            ...
            data = {...
                ...
                'Analytical capacitance,pF',strout.Canl;...
       ...
       function fHelpSetupCallback(~, ~)
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...
            'String', 'This module is an accurate MoM solution for combined metal-dielectric structures with coincident faces. A parallel-plate capacitor is simulated. The module also computes surface charge distributions, electric field distribution in a plane, and electric field at a point.');
