function fig_hdl = E64
%   SYNTAX
%   E64
%   DESCRIPTION
%   This module accurately computes (self) inductance of a helical coil in
%   the presence of a magnetic material (for high mu). Inductance
%   of the air-core coil is computed first. This module also
%   computes "bound charge" distributions, magnetic field distribution in a
%   plane, and magnetic field/scalar potential at a point of interest. The 
%   object's relative permeability should be greater than 100.
%   
...
global snumber;     %   Surface number or domain number for multidomain surfaces (for import only)
...
strcylinder.az         = 1;           %     rotate about the z-axis, deg
strcylinder.mur        = 100;         %    relative magnetic permeability of the cylinder
strcylinder.Tr         = 300;         %    approximate number of triangular patches
strcylinder.par        = 1;
...
%   Parameters for the coil #1
strcoil.center = [0 0 0];            %   coil center, m
...
strcoil.nx      = 0;                 %   axis direction, x
strcoil.ny      = 0;                 %   axis direction, y
strcoil.nz      = 1;                 %   axis direction, z
...
strcoil.CONTOUR = [];                %   contour for all coils
...
strop.planesizex   = 0.1;   %   plane length in m
...
%%  Functions
    function outputgraphics
        ...
        air = 0;
        strout.L = inductance(strcoil, c, csat, P, t, normals, Size, R, strei.mur, mur, snumber, air);
        ...
        graphics_S63(io, strop, stroc, strei, strcoil.ObsCONTOUR, P, t, c);
    end
    ...
    function geometry(objecttype)
        ...
        strcoil.ObsCONTOUR  = zeros(N, 3, turns);
        strcoil.Obst        = [];
        strcoil.ObsP        = [];
        strcoil.Obsnormals  = [];        
        ...
        strout.L = inductance(strcoil, c, csat, P, t, normals, temp, R, strei.mur, mur, snumber, air); 
    ...
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E64-(Self) Inductance of a coil with/without magnetic core (mu_r>100)', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function ExternalCallback(~, ~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            local = figure('Units', Position_units, 'Position', Position_local);
            set(local, 'Name', 'Coil parameters', 'NumberTitle', 'off', 'MenuBar', 'none');
            b1 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Update','Position', Position_b1, 'Parent', local, 'Callback', @B1_Callback);
            b2 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Cancel', 'Position', Position_b2, 'Parent', local, 'Callback', @B2_Callback);
            b3 = uicontrol('Style', 'Text', 'Units', 'normalized', 'String', {'Coil';'Press ENTER after changing each parameter'}, 'Position', Position_b3, 'Parent', local, 'BackgroundColor', [0.8, 0.8, 0.8]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                      
            data = {...
                'Relative permeability of the external medium',strei.mur;...
                'Coil center - x, m',strcoil.center(1);...
                'Coil center - y, m',strcoil.center(2);...
                'Coil center - z, m',strcoil.center(3);...
                ...
                'Axis direction vector, x',strcoil.nx;...
                'Axis direction vector, y',strcoil.ny;...
                'Axis direction vector, z',strcoil.nz;...
                'Coil current, A', strcoil.I;...
                'Coil inductance (computed), H', strout.L;...
            ...
            function edit_callback(hObject, ~)
                data                 = get(hObject,'Data');
                strei.mur            = data{16};
                strcoil.center(1,1)  = data{17};
                strcoil.center(1,2)  = data{18};
                strcoil.center(1,3)  = data{19};
                strcoil.length       = data{20};
                strcoil.radius       = data{21};
                strcoil.wireradius   = data{22};
                strcoil.turns        = data{23};
                strcoil.nx           = data{24};
                strcoil.ny           = data{25};
                strcoil.nz           = data{26};
                strcoil.I            = data{27};                                                
        ...
        function fOutputDataCallback(~, ~)
            temp = 'Coil inductance (full model), H';            
            if ~handles.simulate 
                temp = 'Coil inductance (air core), H';            
            end
            % Callback function run when_______________________________
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            local = figure('Units', Position_units, 'Position', Position_local);
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...                    
            'String', 'This module computes (self) inductance of a helical coil with a finite number of turns in the presence of a magnetic material (mu_r>100). The material may be a sphere, brick, cylinder, or any more complicated mesh (a yoke). Inductance of the air-core coil is computed first. The module also computes "bound charges", magnetic field in a plane, and magnetic field/scalar potential at a point');
...
function [ ] = graphics_S63(io, strop, stroc, strei, ObsCONTOUR, P, t, c);
...
%     stroc.size = 1;                %   relative marker size versus default size 
    ...
    %% Draw the coil
    for m = 1:size(ObsCONTOUR, 3)
        line(ObsCONTOUR(:, 1, m), ObsCONTOUR(:, 2, m), ObsCONTOUR(:, 3, m), 'Color', 'b', 'LineWidth', 2.5);
    end
