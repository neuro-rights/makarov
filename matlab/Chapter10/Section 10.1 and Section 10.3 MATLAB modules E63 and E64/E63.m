function fig_hdl = E63
%   SYNTAX
%   E63
%   DESCRIPTION
%   This module accurately computes (self) inductance of a helical coil in
%   the presence of a magnetic material (for low/moderate mu). Inductance
...
%   object's relative permeability should be less than 100.
%   
...
strsphere.mur      = 10;           %   relative magnetic permeability of the sphere
...
strbrick.mur       = 10;           %    relative magnetic permeability of the brick
...
strcylinder.mur        = 10;          %    relative magnetic permeability of the cylinder
...
strimport.mur       = 1;            %     relative magnetic permeability of the imported mesh
...
strcoil.wireradius = 0.0010;         %   coil wire radius, m
...
%%  Functions
    function outputgraphics
        ...
        air = 0;
        strout.L = inductance(strcoil, c, P, t, normals, Size, R, strei.mur, mur, snumber, air);
        ...
        %%  Find the H-field/B-field and potential at observation point(s)
        ...
        if strcmp(stroc.yes, 'yes')
            ...
            H = hfield(points, c, P, t, Center, Area, normals, Size, R, mu0*strei.mur, msg);
            %%  Coil structure 
...
%            phi = potential(points, c, P, t, Center, Area, normals, Size, R, mu0*strei.mur, msg);
    ...
    function geometry(objecttype)    
        ...
        strout.L = inductance(strcoil, c, P, t, normals, temp, R, strei.mur, mur, snumber, air); 
    ...
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E63-(Self) Inductance of a coil with/without magnetic core (mu_r<100)', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...                    
            'String', 'This module computes (self) inductance of a coil with the finite number of turns in the presence of a magnetic material (mu_r<100). The material may be a sphere, a brick, or a cylinder, or any more complicated mesh (a yoke). Inductance of the air-core coil is computed first. The module also computes "bound charges", magnetic field distribution in a plane, and magnetic field/scalar potential at a point');
...
function [L] = inductance(strcoil, c, P, t, normals, Size, R, muouter, mur, snumber, air) 
    ...
    H = zeros(size(C));
    if ~air
        H = hfield(C, c, P, t, Center, Area, normals, Size, R, muouter*mu0, 2);    
    end   
    Hinc = coil(strcoil, C);
    H = H + Hinc;    
    ...
    % B - inside
    B               = mu0*muouter*H;
    index           = find(outyes==0);    % pointsinside    
    if air
        B(index, :) =  muouter*mu0*H(index, :);
    else
        B(index, :) = mu0*mur*H(index, :);
    end
    %   Flux linkage and inductance (central-point approximation)
    L = 0;    
    for m = 1:length(C)
