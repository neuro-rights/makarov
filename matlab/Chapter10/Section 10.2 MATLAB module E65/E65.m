function fig_hdl = E65
%   SYNTAX
%   E65
%   DESCRIPTION
%   This module accurately computes mutual inductance of two helical coils
%   in the presence of a magnetic material (mu_r>100). Mutual inductance of
%   the air-core coils is computed first. This module also computes
%   "bound charge" distributions, magnetic field distribution in a plane, and
%   magnetic field/scalar potential at a point of interest. The object's
%   relative permeability should be greater than 100.
%   
...
%%  GUI window - major parameters
global P;           %   Surface of the magnetic body
global t;           %   Surface of the magnetic body
global c;           %   Surface charge density
global csat;        %   Second solution
global Area;        %   Magnetic body mesh
global Center;      %   Magnetic body mesh
global normals;     %   Magnetic body mesh
global snumber;     %   Surface number or domain number for multidomain surfaces: 1, 2, etc. (both for import and internal)
global Size;        %   Effective triangle size - base mesh
global Points;      %   Nodes on the observation plane
global mur;         %   Object permeability
...
strcylinder.R          = 0.018;       %     cylinder radius, m
strcylinder.H          = 0.10;        %     cylinder height, m
...
strimport.center1 = [-0.1 0 0];     %     first yoke center, m
strimport.center2 = [+0.1 0 0];     %     second yoke center, m
strimport.ax1         = 0;           %     rotate about the x-axis, deg
strimport.ay1         = 0;           %     rotate about the y-axis, deg
strimport.az1         = 0;           %     rotate about the z-axis, deg
strimport.ax2         = 0;           %     rotate about the x-axis, deg
strimport.ay2         = 0;           %     rotate about the y-axis, deg
strimport.az2         = 0;           %     rotate about the z-axis, deg
strimport.mur       = 100;          %     relative magnetic permeability of the imported mesh

%   Parameters for the external medium (uitable)
strei.mur = 1.0;                    %   relative magnetic permeability of the external medium

%   Parameters for the coil #1 and coil #2
strcoil.center1 = [-0.1 0 0];        %   coil center, m
strcoil.center2 = [+0.1 0 0];        %   coil length, m
strcoil.length = 0.05;              %   coil length, m
strcoil.radius = 0.02;               %   coil radius, m
strcoil.wireradius = 0.001;          %   coil wire radius, m
strcoil.turns  = 7;                  %   number of turns 
strcoil.nx1      = 1;                %   axis direction, x
strcoil.ny1      = 0;                %   axis direction, y
strcoil.nz1      = 0;                %   axis direction, z
strcoil.nx2      = 1;                %   axis direction, x
strcoil.ny2      = 0;                %   axis direction, y
strcoil.nz2      = 0;                %   axis direction, z
strcoil.I       = 1;                 %   coil current, A
strcoil.N       = 400;               %   approximate number of triangles
strcoil.par     = 0.2;               %   mesh nonuniformity parameter
strcoil.ObsP   = [];                 %   for flux integration
strcoil.Obst   = [];                 %   for flux integration
strcoil.Obsnormals = [];             %   for flux integration
strcoil.CONTOUR1 = [];               %   contour for all coils
strcoil.CONTOUR2 = [];               %   contour for all coils

%   Visualization of the H-field in the observation plane (uitable)
strop.yes = 'yes';          %   include (yes) or not (no) plot of the H-field
strop.potential = 'no';     %   include (yes) or not (no) plot of the magnetic potential
strop.planetype   = 'xz';   %   xy, xz, or yz plane
strop.planex = +0.075;      %   plane center: x in m
strop.planey = 0.0;         %   plane center: y in m
strop.planez = 0.0;         %   plane center: z in m
strop.planesizex   = 0.10;  %   plane length in m
strop.planesizey   = 0.15;  %   plane width in m
strop.divisionsx   = 11;    %   divisions with respect to length
strop.divisionsy   = 19;    %   divisions with respect to width
strop.arrow = 1;            %   relative arrow size versus default size
strop.Tr = 200;
...
stroc.z           = 0.0;        %   z position in m (was 0.08)
...
strout.PatchesPerCircle= 0;       %    Total number of triangular patches in the circle mesh for flux calculation
...
strout.M               = 0;       %    Mutual coil inductance
...
%%  Functions
    function outputgraphics
        ...
        %%  Computing inductance (magnetic core)        
        air = 0;
        strout.M = inductance(strcoil, c, csat, P, t, normals, Size, R, strei.mur, mur, snumber, air);

        %%  Find the H-field in a plane (altered)
          if strcmp(strop.yes, 'yes')             
            msg = 1;
            H = hfield(strop.Points, c, P, t, Center, Area, normals, Size, R, mu0*strei.mur, msg);
            %%  Coil structure 
            strop.H = coil(strcoil, strop.Points);
            strop.H = strop.H  + H;
            %%  Cancellation
            h    = waitbar(0, 'Please wait - computing in/out status'); M = size(Points, 1); 
            for m = 1:M
                strop.outyes(m) = inout(Points(m, :), P, t, normals, Center, snumber);
                waitbar(m/M);
            end           
            close(h);                        
            H0              = zeros(size(H));
            index           = find(strop.outyes==0);
            Htemp           = hfield(strop.Points(index, :), csat, P, t, Center, Area, normals, Size, R, mu0*strei.mur, msg);
            H0(index, :)    = Htemp;            
            strop.H(index, :) = H(index, :) - H0(index, :);
          end
        
        %%  Find the potential in a plane (not altered)
        if strcmp(strop.potential, 'yes')
            msg = 1;
            Potential = potential(Points, c, P, t, Center, Area, normals, Size, R, mu0, msg);
            strop.Potential = Potential;
        end
        
        %%  Find the H-field/B-field and potential at observation point(s) (altered)
        ...
        if strcmp(stroc.yes, 'yes')
            ...
            %%  Coil structure 
            Hinc = coil(strcoil, points);      
            if  ~outyes
                e0 = hfield(points, csat, P, t, Center, Area, normals, Size, R, mu0*strei.mur, msg);
                strout.H = e - e0;
            else                   
                strout.H = Hinc + e;                
            end      
        ...
        graphics_S63(io, strop, stroc, strei, strcoil, P, t, c);
    end
    ...
    function geometry(objecttype)        
        %%   De finition of nodal points (arrays P, t treated as cell arrays)
        ...
        if strcmp(objecttype, 'cylinder')
            [P0, t0]  = cylinder(strcylinder.R, strcylinder.H, strcylinder.Tr, strcylinder.par);
            %   First cylinder
            P1 = rotate(P0, strcoil.nx1, strcoil.ny1, strcoil.nz1);  
            [Center1, normals1] = normcenters(P1, t0);
            P1(:, 1) = P1(:, 1) + strcoil.center1(1);
            P1(:, 2) = P1(:, 2) + strcoil.center1(2);
            P1(:, 3) = P1(:, 3) + strcoil.center1(3);            
            Center1(:, 1) = Center1(:, 1) + strcoil.center1(1);
            Center1(:, 2) = Center1(:, 2) + strcoil.center1(2);
            Center1(:, 3) = Center1(:, 3) + strcoil.center1(3);             
            %   Second cylinder
            P2 = rotate(P0, strcoil.nx2, strcoil.ny2, strcoil.nz2); 
            [Center2, normals2] = normcenters(P2, t0); 
            P2(:, 1) = P2(:, 1) + strcoil.center2(1);
            P2(:, 2) = P2(:, 2) + strcoil.center2(2);
            P2(:, 3) = P2(:, 3) + strcoil.center2(3);            
            Center2(:, 1) = Center2(:, 1) + strcoil.center2(1);
            Center2(:, 2) = Center2(:, 2) + strcoil.center2(2);
            Center2(:, 3) = Center2(:, 3) + strcoil.center2(3);   
            %   Combine meshes
            P = [P1; P2];
            t = [t0; t0+size(P1, 1)];
            ...
            snumber = [];
            Tr = size(t0, 1); snumber(1:Tr) = 1; snumber(Tr+1:2*Tr) = 2;                                    
            mur = strcylinder.mur;
        end
        if strcmp(objecttype, 'import')
            ...
            if strcmp(strimport.name, 'none')
                ...
                if ~isequal(file, 0)                
                    TEMP   = load(strcat(path, file), '-mat'); P0 = TEMP.P; t0 = TEMP.t; normals0 = TEMP.normals; snumber = TEMP.snumber;  
                    strimport.name = file;
                else
                ...
                cd 'Meshes/';
                TEMP   = load(strimport.name,'-mat'); P0 = TEMP.P; t0 = TEMP.t; normals0 = TEMP.normals; snumber = TEMP.snumber;
                cd ..       
            end
            %   First core
            t1 = t0;
            P1 = rotatex(P0, strimport.ax1);
            P1 = rotatey(P1, strimport.ay1);
            P1 = rotatez(P1, strimport.az1);   
            normals1 = rotatex(normals0, strimport.ax1);
            normals1 = rotatey(normals1, strimport.ay1);
            normals1 = rotatez(normals1, strimport.az1);                                                
            P1(:, 1) = P1(:, 1) + strimport.center1(1);
            P1(:, 2) = P1(:, 2) + strimport.center1(2);
            P1(:, 3) = P1(:, 3) + strimport.center1(3);    
            Center1  = 1/3*(P1(t1(:, 1),:)+P1(t1(:, 2),:)+P1(t1(:, 3),:));
            %   Second core
            t2 = t0;
            P2 = rotatex(P0, strimport.ax2);
            P2 = rotatey(P2, strimport.ay2);
            P2 = rotatez(P2, strimport.az2);   
            normals2 = rotatex(normals0, strimport.ax2);
            normals2 = rotatey(normals2, strimport.ay2);
            normals2 = rotatez(normals2, strimport.az2);               
            P2(:, 1) = P2(:, 1) + strimport.center2(1);
            P2(:, 2) = P2(:, 2) + strimport.center2(2);
            P2(:, 3) = P2(:, 3) + strimport.center2(3);    
            ...
            normals = [normals1; normals2];
            snumber = [];
            Tr = size(t1, 1); snumber(1:Tr) = 1; snumber(Tr+1:2*Tr) = 2;              
            mur     = strimport.mur;            
        end           
        ...
        %% Coil mesh(es)
        %   This function computes the combined surface mesh (ObsP, Obst, Obsnormals) 
        %   inside all turns of the coil for magnetic flux linkage calculation
        %   It also computes the wire mesh
        %   Coil is rotated first and is moved next   
        h    = waitbar(0, 'Please wait - computing surface meshes for coil turns');
        waitbar(0.5);        
        %%   First coil (master)
        center          = strcoil.center1;
        length          = strcoil.length;
        radius          = strcoil.radius;
        wireradius      = strcoil.wireradius;
        turns           = strcoil.turns; 
        nx              = strcoil.nx1;
        ny              = strcoil.ny1;
        nz              = strcoil.nz1;
        ...
        ny      = ny/temp;
        nz      = nz/temp;
        positions = linspace(-length/2, length/2, turns);   %   linear positions       
        strcoil.ObsCONTOUR1  = zeros(N, 3, turns);
        
        for m = 1:turns
            Contour = [];
            localcenter     = positions(m)*[nx ny nz] + center;  % local center for a loop     
            %   Contour
            phi = linspace(0, 2*pi, N);  %   parameteric ellipse form
            dphi = phi(2) - phi(1); phi = phi + dphi/2; phi(end) = [];                  
            Contour(1, :) = radius*cos(phi);
            Contour(2, :) = radius*sin(phi);
            Contour(3, :) = 0;
            Contour(:, end+1) = Contour(:, 1);    
            Contour  = rotate(Contour', nx, ny, nz);  
            Contour  = Contour + repmat(positions(m)*[nx ny nz]+center, N, 1);         
            strcoil.ObsCONTOUR1(:, :, m) = Contour;                      
        end      
        %%   Second coil
        center          = strcoil.center2;
        length          = strcoil.length;
        radius          = strcoil.radius;
        wireradius      = strcoil.wireradius;
        turns           = strcoil.turns; 
        ...
        nz      = nz/temp;
        positions = linspace(-length/2, length/2, turns);   %   linear positions       
        strcoil.ObsCONTOUR2  = zeros(N, 3, turns);
        strcoil.Obst        = [];
        strcoil.ObsP        = [];
        strcoil.Obsnormals  = [];        
        %   Non-uniform mesh per circle
        factor = 0.75;   %   critical
        [Pcoil, tcoil] = circle(radius-factor*wireradius, strcoil.N, strcoil.par, 0, 0, 1);       
        ...
        for m = 1:turns
            ...
            Contour  = rotate(Contour', nx, ny, nz);  
            Contour  = Contour + repmat(positions(m)*[nx ny nz]+center, N, 1);         
            strcoil.ObsCONTOUR2(:, :, m) = Contour;        
            %   Mesh set inside each turn for flux calculations   
            P0       = rotate(Pcoil, nx, ny, nz);
            P0       = P0 + repmat(positions(m)*[nx ny nz]+center, size(Pcoil, 1), 1); 
            normals0 = repmat([0 0 1], size(tcoil, 1), 1);         
            normals0 = rotate(normals0, nx, ny, nz);
            strcoil.Obst        = [strcoil.Obst; tcoil+size(strcoil.ObsP, 1)];
            strcoil.ObsP        = [strcoil.ObsP; P0];
            strcoil.Obsnormals  = [strcoil.Obsnormals; normals0];           
        end                      
        close(h);
        %% End of coil meshes
        ...
        %%  Inicident field 
        strop.H = coil(strcoil, strop.Points);     
        strout.PatchesPerCircle = size(strcoil.Obst, 1)/strcoil.turns;
        
        %%  Incident field at observation point and field parameters there
        points = [stroc.x stroc.y stroc.z];
        Hinc = coil(strcoil, points);                      
        strout.H = Hinc;               
        outyes = inout(points, P, t, normals, Center, snumber);         
        ...
        %%  Inductance for air-core coil        
        air = 1;
        temp = 1e-6*ones(size(t, 1), 1);
        strout.M = inductance(strcoil, c, csat, P, t, normals, temp, R, strei.mur, mur, snumber, air); 
    ...
    function simulate()        
        ...
        Z = +Z/(4*pi*mu0*strei.mur);        
        close(h);
        
        %%  Solve the MoM equations twice - first time for the infinite permeability
        Z0 = Z;
        %%   Add coefficients to MoM matrix            
        for m = 1:length(t)
            Z0(m, m) = 1/(2*mu0*strei.mur);
        end       
        
        %%   Incident field vector (center point integration)
        V = zeros(length(t), 1);
        Hinc = coil(strcoil, Center);     
        for m = 1:length(t)
            V(m) = dot(normals(m, :), Hinc(m, :));
        end               
        %% Charge conservation law
        %   Subtract the last row from the others and replace the last equation by the conservation law
        for m = 1:length(t)-1
            V(m)    = V(m) - V(end);
            Z0(m, :) = Z0(m,: ) - Z0(length(t), :);
        end
        % Last row
        Z0(length(t), :) = ones(1, length(t)).*Area';
        V(end) = 0;
        strout.time1 = cputime - strout.time1;
        
        %%   Solve MoM equations
        if (length(t)>1000)
            h    = waitbar(0, 'Please wait - solving MoM equations for inifinite permeability');
            waitbar(0.5);
        end
        ...
        DIAG = diag(Z0);
        for m = 1:size(Z0, 1)    %   rowwise
            Z0(m, :) = Z0(m, :)/DIAG(m);
        end
        V = V./DIAG;
        
        csat       = Z0\V;               %   solution for charge coefficients
        clear Z0;                        %   clear MoM matrix
        if (length(t)>1000)
            close(h);
        end
        strout.time2 = cputime - strout.time2;
        Charge0  = csat'.*Area';            %   charges for every face
        strout.TotalChargeSat  = sum(Charge0);
        
        %%  Solve the MoM equations twice - second time for the finite permeability
        
           %%   Add coefficients to MoM matrix     
        Z = +Z*(mur - strei.mur)/(mur + strei.mur);
        for m = 1:length(t)
        ...
        %%   Incident field vector (center point integration)      
        for m = 1:length(t)
            V(m) = (mur - strei.mur)/(mur + strei.mur)*dot(normals(m, :), Hinc(m, :));
        end               
        %% Charge conservation law
        %   Subtract the last row from the others and replace the last equation by the conservation law
        for m = 1:length(t)-1
        ...
        %%   Solve MoM equations
        if (length(t)>1000)
            h    = waitbar(0, 'Please wait - solving MoM equations for finite permeability');
            waitbar(0.5);
        end
    ...
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E65 - Mutual inductance of two coils with/without magnetic core (mu_r>100)', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function fConductingImportCallback(~, ~)
            ...
            b1 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Update','Position', Position_b1, 'Parent', local, 'Callback', @B1_Callback);
            b2 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Cancel', 'Position', Position_b2, 'Parent', local, 'Callback', @B2_Callback);
            b3 = uicontrol('Style', 'Text', 'Units', 'normalized', 'String', {'Import mesh setup'; 'Define positions/rotations of both yokes independently';'Press ENTER after changing each parameter'},'position', Position_b3, 'Parent', local, 'BackgroundColor', [0.8, 0.8, 0.8]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...                
                'Change mesh file',strimport.name;...
                'Center position x of the first yoke,m',strimport.center1(1);...
                'Center position y of the first yoke,m',strimport.center1(2);...
                'Center position z of the first yoke,m',strimport.center1(3);...
                'Center position x of the second yoke,m',strimport.center2(1);...
                'Center position y of the second yoke,m',strimport.center2(2);...
                'Center position z of the second yoke,m',strimport.center2(3);...                
                'Rotate the first yoke about the x-axis, deg',strimport.ax1;...
                'Rotate the first yoke about the y-axis, deg',strimport.ay1;...
                'Rotate the first yoke about the z-axis, deg',strimport.az1;...
                'Rotate the second yoke about the x-axis, deg',strimport.ax2;...
                'Rotate the second yoke about the y-axis',strimport.ay2;...
                'Rotate the second yoke about the z-axis, deg',strimport.az2;...                
                'Relative magnetic permeability of both yokes',strimport.mur};
            ColumnName =   {'Name',   'Value'};
            columnformat    = {'char'};
            columneditable  =  [true];
            uitable('Units', 'normalized', 'Position', Position_table,...
            ...
            function edit_callback(hObject, ~)
                data = get(hObject,'Data');    
                strimport.name               = data{15};
                strimport.center1(1)         = data{16};
                strimport.center1(2)         = data{17};
                strimport.center1(3)         = data{18};
                strimport.center2(1)         = data{19};
                strimport.center2(2)         = data{20};
                strimport.center2(3)         = data{21};                             
                strimport.ax1                = data{22};
                strimport.ay1                = data{23};
                strimport.az1                = data{24};
                strimport.ax2                = data{25};
                strimport.ay2                = data{26};
                strimport.az2                = data{27};                
                strimport.mur                = data{28};               
            end
        ...
        function ExternalCallback(~, ~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            local = figure('Units', Position_units, 'Position', Position_local);
            set(local, 'Name', 'Coils: geometry', 'NumberTitle', 'off', 'MenuBar', 'none');
            b1 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Update','Position', Position_b1, 'Parent', local, 'Callback', @B1_Callback);
            b2 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Cancel', 'Position', Position_b2, 'Parent', local, 'Callback', @B2_Callback);
            b3 = uicontrol('Style', 'Text', 'Units', 'normalized', 'String', {'Coils';'Press ENTER after changing each parameter'}, 'Position', Position_b3, 'Parent', local, 'BackgroundColor', [0.8, 0.8, 0.8]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                      
            data = {...
                'Relative permeability of the external medium',strei.mur;...
                'Coil #1 center - x, m',strcoil.center1(1);...
                'Coil #1 center - y, m',strcoil.center1(2);...
                'Coil #1 center - z, m',strcoil.center1(3);...
                'Coil #2 center - x, m',strcoil.center2(1);...
                'Coil #2 center - y, m',strcoil.center2(2);...
                'Coil #2 center - z, m',strcoil.center2(3);...                
                'Coil length, m',strcoil.length;...
                'Coil radius, m',strcoil.radius;...
                'Coil wire radius, m',strcoil.wireradius;...
                'Number of turns',strcoil.turns;...
                'Coil #1 axis direction vector, x',strcoil.nx1;...
                'Coil #1 axis direction vector, y',strcoil.ny1;...
                'Coil #1 axis direction vector, z',strcoil.nz1;...
                'Coil #2 axis direction vector, x',strcoil.nx2;...
                'Coil #2 axis direction vector, y',strcoil.ny2;...
                'Coil #2 axis direction vector, z',strcoil.nz2;...                
                'Coil current, A', strcoil.I;...
                'Mutual coil inductance (computed), H', strout.M;...
                'Desired number of patches per circle',strcoil.N;...
                'Mesh nonuniformity parameter',strcoil.par};
            ColumnName =   {'Name',   'Value'};
            columnformat    = {'char'};
            columneditable  =  [true];
            uitable('Units', 'normalized', 'Position', Position_table,...
            ...
            function edit_callback(hObject, ~)
                data                    = get(hObject,'Data');
                strei.mur               = data{22};
                strcoil.center1(1,1)    = data{23};
                strcoil.center1(1,2)    = data{24};
                strcoil.center1(1,3)    = data{25};
                strcoil.center2(1,1)    = data{26};
                strcoil.center2(1,2)    = data{27};
                strcoil.center2(1,3)    = data{28};                
                strcoil.length          = data{29};
                strcoil.radius          = data{30};
                strcoil.wireradius      = data{31};
                strcoil.turns           = data{32};
                strcoil.nx1             = data{33};
                strcoil.ny1             = data{34};
                strcoil.nz1             = data{35};
                strcoil.nx2             = data{36};
        ...
        function fOutputDataCallback(~, ~)
            temp = 'Mutual coil inductance (full model), H';            
            if ~handles.simulate 
                temp = 'Mutual coil inductance (air core), H';            
            end
            % Callback function run when_______________________________
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            local = figure('Units', Position_units, 'Position', Position_local);
            ...
            data = {...
                ...
                'Number of triangular patches per area of one coil turn',strout.PatchesPerCircle;...
            ...
            if (strcmp(stroc.yes,'no'))== 1; data{19:28} = 'N.A.'; end;                                 
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...                    
            'String', 'This module computes mutual inductance of two arbitrary helical coils in the presence of a magnetic material (mu_r>100). The material may be a sphere, brick, cylinder, or any more complicated mesh (a yoke). Inductance of the air-core coil is computed first. The module also computes "bound charge" distributions, magnetic field distribution in a plane, and magnetic field/scalar potential at a point');
...
function [ ] = graphics_S63(io, strop, stroc, strei, strcoil, P, t, c);
    ...
    if strcmp(strop.yes, 'yes')
        ...
        normalize = sqrt(max(dot(strop.H, strop.H, 2)))/strop.arrow;
...
%     stroc.size = 1;                %   relative marker size versus default size 
    ...
    %% Draw the coil    
    for m = 1:size(strcoil.ObsCONTOUR1, 3)
        line(strcoil.ObsCONTOUR1(:, 1, m), strcoil.ObsCONTOUR1(:, 2, m), strcoil.ObsCONTOUR1(:, 3, m), 'Color', 'r', 'LineWidth', 2.5);
    end
    for m = 1:size(strcoil.ObsCONTOUR2, 3)
        line(strcoil.ObsCONTOUR2(:, 1, m), strcoil.ObsCONTOUR2(:, 2, m), strcoil.ObsCONTOUR2(:, 3, m), 'Color', 'b', 'LineWidth', 2.5);
    end
    hold on
    %%   Input graphics (within the main GUI window)    
    if io == 0
        colormap jet;
        if ind<IND ec='none'; else ec='none'; end
        h = fill3(X, Y, Z, [0 0 1], 'FaceAlpha', 0.1, 'FaceColor', [0 0.7 0], 'EdgeColor', ec); 
        xlabel('x, m'), ylabel('y, m'), zlabel('z, m'); title('Problem geometry: induction coil and a magnetic object under study');
        ...
        if strcmp(strop.yes, 'yes')
            ...
            C = cv(ttemp(1:3, :));    
            h = fill3(X, Y, Z, C, 'FaceAlpha', 1.0, 'EdgeColor', 'none');
...
function H = hfield(Points, c, P, t, centers, areas, normals, sizes, R, mu, msg)
...
%   SNM Summer 2012
    ...
    if msg ==2
        h    = waitbar(0, 'Please wait - computing H-field for inductance calculation');
    end
...
function [H] = coil(strcoil, r0);
    %   This function computes:
    %   Magnetic field of a coil (coil current I) for a point array r0
    %   Coil is rotated first and is moved next
    %   SNM Spring 2014
    I        = strcoil.I;   
    center   = strcoil.center1;
    ...
    nz       = strcoil.nz1;
    N        = strcoil.N;
    NumberOfCircleSubdivisions = 5*round(sqrt(N));
    ...
    for m = 1:turns
        ...
        [A, Hturn] = staticsh(radius, radius, localcenter, r0, NumberOfCircleSubdivisions, nx, ny, nz);
        H = H + Hturn;      
    end
    H = I*H;
end
...
function [P, t] = circle0(R, N)
    %   Create a mesh for a base circle of the radius R and
    %   with approximately (0.4*N)^2 triangles in the xy-plane (z=0)
    %   SNM Summer 2012
...
function [Pnew, t] = laplace(P, I, type, R);
%   Iterative mesh smoothing based on existing Delaunay connectivity 
%   P - array of vertices
...
function [L] = inductance(strcoil, c, csat, P, t, normals, Size, R, muouter, mur, snumber, air) 
    %   Compute inductance with/without magnetic body
    %   SNM Spring 2014
    h    = waitbar(0, 'Please wait - calculating the inductance');
    waitbar(0.5);
    mu0         = 1.25663706e-006;                  %   magnetic permeability of vacuum(~air)    
    
    %   Disks
    d12 = strcoil.ObsP(strcoil.Obst(:,2),:)-strcoil.ObsP(strcoil.Obst(:,1),:);
    d13 = strcoil.ObsP(strcoil.Obst(:,3),:)-strcoil.ObsP(strcoil.Obst(:,1),:);
    temp = cross(d12, d13, 2);
    A = 0.5*sqrt(dot(temp, temp, 2));  
    C = 1/3*(strcoil.ObsP(strcoil.Obst(:, 1), :) + strcoil.ObsP(strcoil.Obst(:, 2), :) + strcoil.ObsP(strcoil.Obst(:, 3), :));
    
    %   Magnetic body
    d12  = P(t(:,2),:)-P(t(:,1),:);
    d13  = P(t(:,3),:)-P(t(:,1),:);
    temp = cross(d12, d13, 2);
    Area = 0.5*sqrt(dot(temp, temp, 2));    
    Center = 1/3*(P(t(:, 1), :) + P(t(:, 2), :) + P(t(:, 3), :));
    
     %Inout status
    outyes = zeros(size(C, 1), 1);    
    for m = 1:length(C)        
        outyes(m) = inout(C(m, :), P, t, normals, Center, snumber);
    end    
    index           = find(outyes==0);    % points inside  
    
    H = zeros(size(C));    
    if ~air      
        e  = hfield(C, c, P, t, Center, Area, normals, Size, R, muouter*mu0, 2); 
        e0 = hfield(C(index, :), csat, P, t, Center, Area, normals, Size, R, muouter*mu0, 2); 
        Hinc = coil(strcoil, C);
        H = e + Hinc;
        H(index, :) = e(index, :) - e0;                
    else    
    ...
    B               = mu0*muouter*H;    
    if air
        B(index, :) =  muouter*mu0*H(index, :);
    else
        B(index, :) = mu0*mur*H(index, :);
    end
    %   Inductance - central-point approximation
    L = 0;    
    for m = 1:length(C)
        L = L + A(m)*sum(B(m, :).*strcoil.Obsnormals(m, :));
    end
    L = L/strcoil.I;      
    close(h);
end    
