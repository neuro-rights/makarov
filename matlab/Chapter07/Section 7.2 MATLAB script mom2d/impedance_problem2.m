...
%   a microstrip air-filled TL via the MoM method
%
...
h       = w/100;            % Edge length (variable parameter)

% X- and Y-coordinates for starting and ending points
% Bottom plate (X times top plate)
X = 10; W = X*w; H = X*h;
xs_b  = [-W/2:H:W/2-H]; 
xe_b  = xs_b + H;
...
Z0          = 1/(const.c*C0)                % In Ohm
Error       = abs(sum(charge))/max(abs(charge))
[Z0_Pozar]   = imp_microstrip(w, d, 1)
