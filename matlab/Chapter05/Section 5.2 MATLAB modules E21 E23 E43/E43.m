function fig_hdl = E43
%   SYNTAX
%   E43
%   DESCRIPTION
%   This module accurately computes capacitance of a capacitor including a
%   metal conductor and a human body. 19 body meshes from three real
%   subjects are available. In the DC steady state, the internal structure
%   of the body is not significant since the field inside the body is
%   exactly zero. The module also computes electric field distribution in a
%   plane and at a point of interest.
%   
...
strge.Tr(1)             = 1600;        %   approximate number of triangular patches
...
strop.planez = 0.95;        %   plane center: z in m
strop.planesizex   = 1.0;   %   plane length in m
strop.planesizey   = 1.7;   %   plane width in m
...
strout.ChargeMetal      = 0;                     %    Total charge of the lower plate, C
strout.ChargeObject     = 0;                     %    Total charge of the upper plate, C
strout.E(1,1)           = 0;
strout.E(1,2)           = 0;
...
%%  Functions
    ...
    function geometry()        
        ...
        if ~strcmp(objecttype{2}, 'person1_phone_pose.mat')
            TEMP = load(objecttype{2}, '-mat');
        end
        PD{2} = TEMP.P;
        tD{2} = TEMP.t;
        tD{2}(:, 4) = 3;     
        normalsD{2} = TEMP.normals;
    ...
    function simulate()
        ...
        ChargeMetal     = sum(Charge(t(:, 4)==1));    %   total charge of the conductor, C
        ChargeObject    = sum(Charge(t(:, 4)==3));    %   total charge of the body, C
        ChargeTotal     = ChargeMetal + ChargeObject;
        Cnum            = sum(Charge(t(:, 4)==1))*1e12/(objectpotential{1}-objectpotential{2});
        Ctheory         = 'Undefined';
        ...
        strout.ChargeMetal     = ChargeMetal;       %    Total charge of the metal conductor, C
        ...
        strout.PatchesTotal;                        %    Total number of triangular patches in themesh
        strout.quality  = min(simpqual(P, t));
...
%% ---------------------------------------------------------------------------
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E43 - A capacitor formed by a human body and a metal conductor - MoM/BEM numerical solution', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
...
%                 end
        ...
        function fOutputDataCallback(~, ~)
            ...
            data = {...
                ...
                'Total charge of the metal conductor, C',strout.ChargeMetal;...                
       ...
       % --- View Direction ---
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...            
            'string','This module accurately computes capacitance of a capacitor which consists of a metal conductor and a human body. Human-body potential is fixed at 0V. 19 body meshes from three real subjects are available. In DC steady state, the internal structure of the body is not significant since the field inside the body is exactly zero. The module also computes electric field distribution in a plane and at a point of interest.');
...
%     stroc.size = 1;                %   relative marker size versus default size 
    ...
    if io == 0
        ...
        xlabel('x, m'), ylabel('y, m'), zlabel('z, m'); 
        title('Problem geometry: metal conductor and human body');
    end
