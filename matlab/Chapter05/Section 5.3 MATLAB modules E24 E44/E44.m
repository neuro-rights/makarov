function fig_hdl = E44
%   SYNTAX
%   E44
%   DESCRIPTION
%   This module accurately computes fields and free surface charges for a
%   human body in an external electric field formed by two arbitrary metal
%   conductors. 19 body meshes from three real subjects are available. In
%   the DC steady state, the internal structure of the body is not
%   significant since the field inside the body is exactly zero. The module
%   also computes electric field distribution in a plane and at a point of
%   interest.
%   
...
global Size;
global Points;
global TEMP; TEMP   = load('person1_phone_pose.mat','-mat'); 

%   Parameters for conducting objects (uimenu)
objecttype{1}           = 'plate';     %   'none' or 'plate', 'sphere', 'brick', or 'cylinder' is allowed
objecttype{2}           = 'plate';     %   'none' or 'plate', 'sphere', 'brick', or 'cylinder' is allowed
objecttype{3}           = 'person1_phone_pose.mat';
...
objectpotential{3}      = 0.0;          %    potential of the third conducting object, V
...
strge.L(1)              = 0.4;         %   plate length, m
strge.W(1)              = 0.4;         %   plate width, m
strge.X(1)              = 0;           %   center position x, m
strge.Y(1)              = -0.25;        %   center position y, m
strge.Z(1)              = 1.2;         %   center position z, m
strge.ax(1)             = 90;          %   rotate about the x-axis, deg
...
strge.Tr(1)             = 200;         %   approximate number of triangular patches

%   Data for the plate (if 'plate' is selected as a second object) (uitable)
strge.L(2)              = 0.4;         %   plate length, m
strge.W(2)              = 0.4;         %   plate width, m
strge.X(2)              = 0;           %   center position x, m
strge.Y(2)              = 0.25;         %   center position y, m
strge.Z(2)              = 1.2;         %   center position z, m
strge.ax(2)             = 90;          %   rotate about the x-axis, deg
...
strsphere.a(1)          = 0.2;         %   sphere radius, m
strsphere.X(1)          = 0;            %   center position x, m
strsphere.Y(1)          = 0.5;          %   center position y, m
...
strsphere.a(2)          = 0.2;        %   sphere radius, m
strsphere.X(2)          = 0.0;        %   center position x, m
strsphere.Y(2)          = -0.5;       %   center position y, m
strsphere.Z(2)          = 0;          %   center position z, m
...
strbrick.L(1)          = 1;         %    brick length, m
strbrick.W(1)          = 1;         %    brick width, m
strbrick.H(1)          = 0.1;      %    brick height, m
strbrick.X(1)          = 0;          %    center position x, m
strbrick.Y(1)          = 0;           %    center position y, m
strbrick.Z(1)          = -0.2;           %    center position z, m
...
strbrick.L(2)          = 1;         %    brick length, m
strbrick.W(2)          = 1;         %    brick width, m
strbrick.H(2)          = 0.1;        %    brick height, m
strbrick.X(2)          = 0;          %    center position x, m
strbrick.Y(2)          = 0;           %    center position y, m
strbrick.Z(2)          = 1.9;           %    center position z, m
...
strcylinder.H(1)          = 2.0;         %     cylinder height, m
strcylinder.X(1)          = 0;           %     center position x, m
strcylinder.Y(1)          = 0.5;         %     center position y, m
strcylinder.Z(1)          = 1.7;         %     center position z, m
...
strcylinder.Tr(1)         = 4;           %     approximate number of triangular patches
strcylinder.par(1)        = 1;

%   Data for the cylinder (if 'cylinder' is selected as a second object) (uitable)
strcylinder.R(2)          = 0.02;        %     cylinder radius, m
strcylinder.H(2)          = 2.0;         %     cylinder height, m
strcylinder.X(2)          = 0.2;         %     center position x, m
strcylinder.Y(2)          = 0.5;        %     center position y, m
strcylinder.Z(2)          = 2.0;         %     center position z, m
...
strcylinder.Tr(2)         = 4;           %     approximate number of triangular patches
strcylinder.par(2)        = 1;
...
strcircle.Z(1)              = 2;           %   center position z, m
...
strop.planez = 1.2;         %   plane center: z in m
strop.planesizex   = 1.0;   %   plane length in m
strop.planesizey   = 1.0;   %   plane width in m
strop.divisionsx   = 16;    %   divisions with respect to length
strop.divisionsy   = 16;    %   divisions with respect to width
strop.arrow = 3.0;          %   relative arrow size versus default size
...
%%  Functions
    function outputgraphics
        ...
        graphics_S44(io, strop, stroc, P, t, c, Area, strsc);
    end
    ...
    function geometry()        
        ...
        if ~strcmp(objecttype{3}, 'person1_phone_pose.mat')
            TEMP = load(objecttype{3}, '-mat');
        end
        PD{3} = TEMP.P;
        tD{3} = TEMP.t;
        tD{3}(:, 4) = 3;     
        normalsD{3} = TEMP.normals;
...
%% ---------------------------------------------------------------------------
    function build_gui()
        ...
        handles.figure1 = figure( ...
            'Tag', 'figure1', ...
            'Units', 'characters', ...
            'Position', [103.8 23.0769230769231 149 38.4615384615385], ...
            'Name', 'E44 - Human body in a DC field formed by two conductors-MoM/BEM numerical solution', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
...
%                 end
        ...
        function fOutputDataCallback(~, ~)
            ...
            data = {...
                ...
                'Average electric potential of the human body, V',strout.ObjectPotential;...
       ...
       % --- View Direction ---
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...            
            'string','This module accurately computes fields and surface charges for a human body in an external electric field formed by arbitrary metal conductors including capacitance variations. 19 body meshes from three real subjects are available. In DC steady state, the internal structure of the body is not significant since the field inside the body is exactly zero. The module also computes electric field distribution in a plane and at a point of interest.');
...
%     stroc.size = 1;                %   relative marker size versus default size 
    ...
    if io == 0
        ...
        light('Position',[1 3 2]); light('Position',[-3 -1 3]); material shiny; grid on;
        xlabel('x, m'), ylabel('y, m'), zlabel('z, m');  
        title('Problem geometry: two conductors and the conducting (uncharged) human body');
    end
...
%   SNM Summer 2012
    ...
    if msg ==3
        h    = waitbar(0, 'Please wait - computing average electric potential of human body');
    end
