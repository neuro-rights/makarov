function fig_hdl = E22
%   SYNTAX
%   E22
%   DESCRIPTION
%   This module accurately computes fields and free surface charges for a
%   conducting object in an external electric field. The object may be a
%   sphere, a brick, or a cylinder. Arbitrary directions of the incident
%   field may be given. This module also computes charge distributions on
%   object surfaces, total electric field distribution in a plane, and total
%   electric field at a point of interest (marked by a cross)
%   
...
global objecttype; objecttype = 'sphere';  %   either 'sphere' or 'brick' or 'cylinder' is allowed

%   Data for the plate  - first conductor('plate' is always selected) (uitable)
strge.L              = 1.0;         %   plate length, m
strge.W              = 1.0;         %   plate width, m
strge.X              = 0;           %   center position x, m
strge.Y              = 0;           %   center position y, m
strge.Z              = -0.1;        %   center position z, m
strge.ax             = 0;           %   rotate about the x-axis, deg
strge.ay             = 0;           %   rotate about the y-axis, deg
strge.az             = 0;           %   rotate about the z-axis, deg
strge.par            = 1;           %   uniform (0) or non-uniform (1) grid
strge.Tr             = 200;
...
strcylinder.R          = 0.3;         %     cylinder radius, m
...
strop.planesizex   = 1.75;  %   plane length in m
strop.planesizey   = 1.75;  %   plane width in m
strop.divisionsx   = 13;    %   divisions with respect to length
strop.divisionsy   = 13;    %   divisions with respect to width
strop.arrow = 0.7;            %   relative arrow size versus default size
strop.Tr = 500;
...
%%  Functions
    function outputgraphics
        ...
        graphics_S22(io, strop, stroc, strei, P, t, c);
    end
    ...
    function geometry()        
        ...
        if strcmp(objecttype, 'plate')
            [PD{1}, tD{1}] = plate(strge.L, strge.W, strge.Tr, strge.par, 0, 0, 1);
            [PD{1}] = rotatex(PD{1}, strge.ax);
            [PD{1}] = rotatey(PD{1}, strge.ay);
            [PD{1}] = rotatez(PD{1}, strge.az);
            [centersD{1}, normalsD{1}] = normcenters(PD{1}, tD{1});
            PD{1}(:, 1) = PD{1}(:, 1) + strge.X;
            PD{1}(:, 2) = PD{1}(:, 2) + strge.Y;
            PD{1}(:, 3) = PD{1}(:, 3) + strge.Z;
            centersD{1}(:, 1) = centersD{1}(:, 1) + strge.X;
            centersD{1}(:, 2) = centersD{1}(:, 2) + strge.Y;
            centersD{1}(:, 3) = centersD{1}(:, 3) + strge.Z;
        end
        ...
        if strcmp(objecttype, 'cylinder')
            ...
            [centersD{1}, normalsD{1}] = normcenters(PD{1}, tD{1});
            P = PD{1}; t = tD{1}; normals = normalsD{1};
            %save('qq', 'P', 't', 'normals');
            %   movement
...
%% ---------------------------------------------------------------------------
    function build_gui()
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E22-Conducting object in an external electric field - MoM/BEM numerical solution', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function fConductingPlateCallback(~, ~)
            ...
            set(local, 'Name', 'Conducting object', 'NumberTitle', 'off', 'MenuBar', 'none');
            ...
            b3 = uicontrol('Style', 'Text', 'Units', 'normalized', 'String', {'parameters of the plate';'Press ENTER after changing every parameter'}, 'Position', Position_b3, 'Parent', local, 'BackgroundColor', [0.8, 0.8, 0.8]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...
                'Plate length,m',strge.L;...
                'Plate width,m',strge.W;...
                'Center position x,m',strge.X;...
                'Center position y,m',strge.Y;...
                'Center position z,m',strge.Z;...
                'Rotate about x-axis, deg',strge.ax;...
                'Rotate about y-axis, deg',strge.ay;...
                'Rotate about z-axis, deg',strge.az;...
                'Approximate number of triangles',strge.Tr;...
                'Triangle size at boundaries vs. center size', strge.par};
        ...
        function fConductingSphereCallback(~, ~)
            ...
            b1 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Update','Position', Position_b1, 'Parent', local, 'Callback', @B1_Callback);
            b2 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Cancel', 'Position', Position_b2, 'Parent', local, 'Callback', @B2_Callback);
            b3 = uicontrol('Style', 'Text', 'Units', 'normalized', 'String', {'parameters of sphere','Press ENTER after changing every parameter'}, 'Position', Position_b3, 'Parent', local, 'BackgroundColor', [0.8, 0.8, 0.8]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...
        ...
        function fConductingCylinderCallback(~, ~)
            ...
            function edit_callback(hObject, ~)
                data = get(hObject,'Data');
                strcylinder.R         = data{11};
                strcylinder.H         = data{12};
                strcylinder.X         = data{13};
                strcylinder.Y         = data{14};
                strcylinder.Z         = data{15};
                strcylinder.ax        = data{16};
                strcylinder.ay        = data{17};
                strcylinder.az        = data{18};
                strcylinder.Tr =data{19};
                strcylinder.par =data{20};
            end
       ...
       % --- View Direction ---
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...
            'string','This module accurately computes fields and free surface charges for a conducting object in an external electric field. The object may be a sphere, a brick, or a cylinder. Arbitrary directions of the incident field may be given. This module also computes charge distributions on object surface, total electric field distribution in a plane, and total electric field at a point of interest (marked by a cross).');
