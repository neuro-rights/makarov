function fig_hdl = E21a
%   SYNTAX
%   E21a
%   DESCRIPTION
%   This module accurately computes the capacitance of a parallel-plate
%   capacitor including adaptive mesh refinement. It also computes
%   capacitance values at every iteration step
%   
...
strge.Z(1)              = -0.05;       %   center position z, m
...
strop.arrow = 1.75;         %   relative arrow size versus default size

%   Surface charge and general visualization (uitable)
%   Parameters to scale charge distribution for better visual inspection
strsc.positive = 0.5;         %   positive charge densities higher than this number times the average
                              %   positive charge density are assigned the same value
strsc.negative = 0.5;         %   negative charge densities smaller than this number times the average
                              %   negative charge density are assigned the same value

%   Parameters for the observation point(s) (used to get exact values of the field) (uitable)
stroc.yes         = 'no';      %   'yes' - present; 'no' - absent
stroc.x           = 0.0;       %   x position in m
stroc.y           = 0.0;       %   y position in m
stroc.z           = 0.2;       %   z position in m
stroc.size        = 2;         %   relative marker size versus default size
...
gauss = 7;          %   Number of integration points in the Gaussian quadrature
                    %   Numbers 1, 3, 4, 6, 7, 9, 13, 25 are permitted
Iterations = 5;     %   Iteration number for adaptive mesh refinement
Percentage = 15;    %   Percentage of patches added for adaptive mesh refinement
...
strout.Ctheory          = 0;        %    Analytical (C=A*eps0/d) capacitance in pF
...
strout.E(1,1)           = 0;
strout.E(1,2)           = 0;        %    E-field at the observation point (if any)
strout.E(1,3)           = 0;
...
%%  Functions
   function outputgraphics
    ...
    function geometry(strop, stroc)        
        for m = 1:2
            [PD{m}, tD{m}, N{m}] = plate(strge.L(m), strge.W(m), strge.Tr(m), 0, 0, 1);           
    ...
    function simulate()         
        ...
        %   n is the number of the inner triangle (every column has only the n-th inner triangle)
        for n =1:NM
            normdummy    = normals(n, :);
        ...
        %---------------- Corrected charge also has the zero mean----------
        for m = 1:length(t)-1
            dV(m)    = dV(m) - dV(end);        
        end    
        dV(end) = 0;
        ...
        ChargeTotal     = ChargeLower + ChargeUpper;          
        ...
        strout.Ctheory          = min(strge.L)*min(strge.W)*eps0/temp; 
        ...
        strout.AvgPot1          = sum(Area(t(:, 4)==1).*PotentialTr(t(:, 4)==1))/sum(Area(t(:, 4)==1));
        strout.AvgPot2          = sum(Area(t(:, 4)==2).*PotentialTr(t(:, 4)==2))/sum(Area(t(:, 4)==2));  
        strout.CStandard        = 1e12*ChargeUpper/(strge.phiu - strge.phi);
        strout.CAvgPot          = 1e12*ChargeUpper/(strout.AvgPot2 - strout.AvgPot1); 
        C1 = sum((c(t(:, 4)==1).*Area(t(:, 4)==1))./PotentialTr(t(:, 4)==1));
        C2 = sum((c(t(:, 4)==2).*Area(t(:, 4)==2))./PotentialTr(t(:, 4)==2));       
        strout.CLocal           = 1e12*1/(1/C1 + 1/C2); 
...
%% ---------------------------------------------------------------------------
    function build_gui()
        ...
        %global Position_local;  Position_local  = [103.8 23.0769230769231 68 38.4615384615385];
        ...
        handles.figure1 = figure( ...
            ...
            'Name', 'E21a-Capacitance of a parallel-plate capacitor - MoM numerical solution', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        function fFirstParametersMenuCallback(~, ~)
            ...
            b3 = uicontrol('Style', 'Text', 'Units', 'normalized', 'String', {'This is the panel to control the parameters of the first plate';'Press ENTER after changing every parameter'}, 'Position', Position_b3, 'Parent', local, 'BackgroundColor', [0.8, 0.8, 0.8]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...
        ...
        function fSecondParametersMenuCallback(~, ~)
            ...
            b1 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Update','Position', Position_b1, 'Parent', local, 'Callback', @B1_Callback);
            b2 = uicontrol('Style', 'Pushbutton', 'Units', 'normalized', 'String', 'Cancel', 'Position', Position_b2, 'Parent', local, 'Callback', @B2_Callback);
            b3 = uicontrol('Style', 'Text', 'Units', 'normalized', 'String', {'This is the panel to control the parameters of the second plate';'Press ENTER after changing every parameter'}, 'Position', Position_b3, 'Parent', local, 'BackgroundColor', [0.8, 0.8, 0.8]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            data = {...
        ...
        function fOutputSetupCallback(~, ~)
            ...
            function edit_callback(hObject, ~)
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                keepplane = strcmp(strop.planetype, data{21})&...
        ...
        function fModelingSetupCallback(~, ~)
            ...
            data = {...
                ...
                'Percentage of faces added at every step',Percentage;...
        ...
        function fOutputDataCallback(~, ~)
            ...
            data = {...
                ...
                'Analytical capacitance, pF',strout.Ctheory;...
                'Total charge of the lower plate, C',strout.ChargeLower;...
                'Total charge of the upper plate, C',strout.ChargeUpper;...
            ...
            if (strcmp(stroc.yes,'no'))== 1; data{19} = 'N.A.'; data{20} = 'N.A.'; data{21} = 'N.A.'; data{22} = 'N.A.';  end;
        ...
        handles.abstract = uicontrol( ...
            ...
            'FontSize', 8, ...
            'FontUnits', 'pixels', ...
            'string','This module accurately computes the capacitance of a parallel-plate capacitor using adaptive mesh refinement. It also computes capacitance at every iteration step. This module uses the MoM solution correction procedure.');
        ...
        function SimulateCallback(~,~)        
            ...
            fv.faces = t(:, 1:3); fv.vertices = P; patch(fv, 'FaceColor', 'y'); axis equal; view(-39, 42); grid on;
            ...
            for n = 2:Iterations
                ...
                index1 = t(:, 4)==1;
                index2 = t(:, 4)==2;             
                PotentialError1 = abs(PotentialTr(index1) - objectpotential{1}).*Area(index1).^Power;   %   Local potential error for mesh refinement 
                PotentialError2 = abs(PotentialTr(index2) - objectpotential{2}).*Area(index2).^Power;   %   Local potential error for mesh refinement
                %   Order is important
                [P2, t2, N2] = plate_refine(P2, t(index2, :)-size(P1, 1), N2, PotentialError2, Percentage, Refine, Method);                   
                [P1, t1, N1] = plate_refine(P1, t(index1, :),             N1, PotentialError1, Percentage, Refine, Method);    
                P1 = rotatex(P1, strge.ax(1));
                P1 = rotatey(P1, strge.ay(1));
                P1 = rotatez(P1, strge.az(1));            
                P1(:, 1) = P1(:, 1) + strge.X(1);
                P1(:, 2) = P1(:, 2) + strge.Y(1);
                P1(:, 3) = P1(:, 3) + strge.Z(1);                 
                P2 = rotatex(P2, strge.ax(2));
                P2 = rotatey(P2, strge.ay(2));
                P2 = rotatez(P2, strge.az(2));            
                P2(:, 1) = P2(:, 1) + strge.X(2);
                P2(:, 2) = P2(:, 2) + strge.Y(2);
                P2(:, 3) = P2(:, 3) + strge.Z(2); 
                t1(:, 4) = 1;
                t2(:, 4) = 2;
                temp        = t2(:, 1:3) + size(P1, 1);
                temp(:, 4)  = 2;       
                t = [t1; temp];
                P = [P1; P2];
...
function [P, t] = brick(L, W, H, Tr, par)
    ...
    %   base brick with length L, width W, and height H 
...
function [P, t, I] = circle(R, Tri, nx, ny, nz)
    %   Create a mesh for a base circle of radius R and
...
function [P, t, I] = plate(L, W, Tri, nx, ny, nz);
    %   Create a mesh for a base plate of length L and width W
...
function [P, t] = plate0(L, W, Nx, Ny, indicator, nx, ny, nz)
    %   Create structured mesh for a base plate of size L by W with the normal vector nx, ny, nz  
    ...
    %   Rotate mesh
    theta   = acos(nz)*sign(ny + eps);              %   rotation about the x-axis following the right-hand rule
    phi     = asin(nx/sqrt(nx^2+ny^2+eps));         %   rotation about the z-axis 
    theta   = theta/pi*180;
    phi     = phi/pi*180; 
    P       = rotatex(P, theta);
    P       = rotatez(P, phi);
...
% %      Int = potint2(r1, r2, r3, normal, ObsPoint)
    ...
    %   Calculate vectors from observation point projected to midpoints
...
function [coeff, weights, IndexF] = tri(arg1, arg2);
    ...
    % seventh (13 integration points), and tenth (25 integration points)
    % order of accuracy may be created.
    % 
    %
